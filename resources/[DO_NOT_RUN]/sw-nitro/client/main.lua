...
local function NitroLoop(lastVehicle)
	...
	if isRunning and isEnabled and isFueled and hasNOS > 0 then
		hasNOS = hasNOS - 1
		if isDriving then
...
AddEventHandler(
	...
	function(playerServerId, boostEnabled, purgeEnabled, lastVehicle)
		...
		-- check for that, their player ID will be -1. PlayerPedId() is our local
...
RegisterNetEvent("nitro:fillTank")
AddEventHandler(
	"nitro:fillTank",
	function()
		hasNOS = 1000
